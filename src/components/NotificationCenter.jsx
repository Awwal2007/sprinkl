...
export default function NotificationCenter() {
  ...
  const getToastStyles = (type) => {
    switch (type) {
      case 'success':
        return {
          card: 'border-emerald-500/30 bg-white/95 dark:bg-slate-950/95 shadow-emerald-500/15',
          accent: 'bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.5)]',
        };
      case 'error':
        return {
          card: 'border-rose-500/30 bg-white/95 dark:bg-slate-950/95 shadow-rose-500/15',
          accent: 'bg-rose-400 shadow-[0_0_8px_rgba(251,113,133,0.5)]',
        };
      case 'warning':
        return {
          card: 'border-amber-500/30 bg-white/95 dark:bg-slate-950/95 shadow-amber-500/15',
          accent: 'bg-amber-400 shadow-[0_0_8px_rgba(251,191,36,0.5)]',
        };
      default:
        return {
          card: 'border-brand-500/30 bg-white/95 dark:bg-slate-950/95 shadow-brand-500/15',
    ...
    <>
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  {t.title && (
                    <p className="text-[11px] sm:text-xs font-extrabold text-slate-900 dark:text-white tracking-wide mb-0.5 leading-tight">
                      {t.title}
                    </p>
                  )}
                  <p className="text-[11px] sm:text-xs text-slate-600 dark:text-slate-200 leading-relaxed break-words font-medium">
                    {t.message}
                  </p>
                </div>
                ...
                <button
                  ...
                  className="text-slate-400 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white transition-colors p-1 -mr-0.5 -mt-0.5 rounded-lg shrink-0"
      ...
      {confirmModal.isOpen && (
        <div className="fixed inset-0 z-[99998] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-200">
          <div
            className="relative w-full max-w-md bg-white dark:bg-dark-card border border-slate-200 dark:border-dark-border rounded-3xl p-6 sm:p-7 shadow-2xl space-y-5 animate-in zoom-in-95 duration-200"
            role="dialog"
            aria-modal="true"
          >
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              <button
                ...
                className="px-4 py-2.5 rounded-xl border border-slate-200 dark:border-dark-border bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 font-bold text-xs transition-colors"
              >
