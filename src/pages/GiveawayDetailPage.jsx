...
export default function GiveawayDetailPage() {
  ...
  const { data, refetch } = useQuery({
    ...
    refetchInterval: 3000, // Poll every 3 seconds for live claim stream updates
  });
  ...
  const handleCancel = async () => {
    if (!window.confirm('Are you sure you want to cancel this giveaway? Unclaimed funds will be returned to your wallet.')) return;
    try {
      setCancelling(true);
  ...
  return (
    <div className="min-h-screen flex flex-col bg-dark-bg text-slate-100">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-1 w-full space-y-6">
        ...
        <div className="bg-dark-card border border-dark-border rounded-2xl p-6 sm:p-8 space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <h1 className="text-2xl sm:text-3xl font-extrabold text-white">{giveaway.title}</h1>
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="px-4 py-2.5 bg-brand-500 hover:bg-brand-600 text-slate-950 font-bold text-xs rounded-xl flex items-center gap-1.5 transition-all shadow-lg shadow-brand-500/20"
              >
              ...
              {giveaway.status === 'active' && (
                <button
                  ...
                  disabled={cancelling}
                  className="px-3.5 py-2.5 bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/20 text-rose-400 text-xs font-bold rounded-xl flex items-center gap-1.5 transition-colors disabled:opacity-50"
                >
        ...
        {/* Real-time Claims Feed Table */}
        <section className="bg-dark-card border border-dark-border rounded-2xl p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-lg font-bold text-white">Live Claim Stream</h2>
              <p className="text-xs text-dark-muted">Real-time payouts initiated to claimants</p>
            </div>
            <span className="text-[10px] bg-brand-500/10 text-brand-400 border border-brand-500/20 px-2 py-0.5 rounded font-bold animate-pulse">
              LIVE UPDATING
            </span>
          </div>
                ...
                <tbody className="divide-y divide-dark-border text-xs">
                  {claims.map((c) => (
                    <tr key={c._id} className="hover:bg-slate-800/30">
                      ...
                      <td className="py-3 px-3 font-mono text-dark-muted">
                        {c.currency === 'NGN' ? `${c.destination.bankName} - ${c.destination.accountNumber}` : c.destination.walletAddress}
                      </td>
